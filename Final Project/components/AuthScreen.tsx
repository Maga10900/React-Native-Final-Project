...
export default function AuthScreen({
  ...
  return (
    <View className="flex-1 bg-white pt-10 px-6">
      ...
      >
        <Text className="text-[32px] font-black text-[#1E1E1E] mb-2">
          {title}
        </Text>
        ...
        {fields.map((field, index) => {
          ...
          return (
            <View key={field.name} className={containerClass}>
              <Text className="text-sm font-bold text-[#1E1E1E] mb-2">
                {field.label}
              </Text>
              <View
                className={`flex-row items-center border ${field.error ? "border-red-500" : "border-[#E7E7E7]"} rounded-xl px-4 py-3 bg-[#FAFAFA]`}
              >
                <Ionicons
                  name={field.icon}
                  size={20}
                  color={field.error ? "#EF4444" : "#999"}
                />
                {field.isSelect ? (
                  <TouchableOpacity
                    className="flex-1 ml-3"
                    onPress={field.onPressSelect}
                  >
                    <Text
                      className={`text-base ${field.value ? "text-[#1E1E1E]" : "text-[#999]"}`}
                    >
        ...
        >
          <Text className="text-[#1E1E1E] font-black text-lg">
            {buttonText}
          </Text>
        </TouchableOpacity>
        ...
        <View className="flex-row justify-center mt-8">
          ...
          <TouchableOpacity onPress={onFooterActionPress}>
            <Text className="text-[#1E1E1E] font-black text-base underline">
              {footerActionText}
            </Text>
          </TouchableOpacity>
        </View>
